...
def Gaussian_kernel(X1, X2, sigma):
    ...
    return torch.exp(1/(-2 * np.power(sigma, 2)) * (X_row.repeat(1, len(X_col)) + X_col.repeat(len(X_row), 1) - 2 * X1.mm(X2.T)))

def linear_regression(X_train, y_train, D = 200, lambda_reg = 0.01):
    # X_train = X_train.reshape(-1, D)
    # y_train = y_train.reshape(y_train.shape[0] * y_train.shape[1], -1)

    W = torch.matmul(torch.linalg.inv(torch.matmul(X_train.T, X_train) + lambda_reg * torch.eye(D, device=device) * X_train.shape[0]), torch.matmul(X_train.T, y_train.float()))
    return W
...
def initial_weights(d, D, K, bp = True, std1 = 0.05, std2 = 0.05, stationary=False, device = 'cuda:0', dtype = torch.float):
    ...
    if stationary:
        ...
        Omega1 = torch.empty(d, D, device = device, r=bp)
...
def feature_mapping(X, parameters, stationary):    
    if stationary :
        ...
        phi = torch.cos(X.mm(Omega1) + b1.repeat(X.shape[0], 1))*math.sqrt(1/Omega1.shape[1])
    else:
...
def askl(parameter_dic):
    ...
    local_update = parameter_dic['local_update']

    # Load data
    trainloader, validateloader, testloader, d, K = load_data(dataset, batch_size)
    X_val, y_val = iter(validateloader).next()
    if K == 1:
        loss_type = 'mse'
    elif K == 2:
        loss_type = 'hinge'
    ...
    for epoch in range(T):
        ...
        for i_batch, train_batch in enumerate(trainloader, 0):
            ...
            if i_batch % local_update == 0:
                PhiX_val = feature_mapping(X_val.to(device), spectral_measure, stationary)
                regularaztion_phi = lambda_B * torch.norm(PhiX_val, 'fro') / PhiX_val.shape[0]
                # regularaztion_phi = lambda_B * torch.norm(phi, 'fro')
                objective  += regularaztion_phi
                training_regularaztion_phi += regularaztion_phi.item()
            
            # Records
            training_loss += loss.item()
            training_regularaztion_W += regularaztion_W.item()
            # training_regularaztion_phi += regularaztion_phi.item() if regularaztion_phi == None else 
            ...
            # Backward
            objective.backward()
            if regular_type == 'nuc' and lambda_A > 0:
                with torch.no_grad():
                    U, S, V = torch.svd(W)
